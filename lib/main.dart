...
class _MyAppState extends State<MyApp> {
      ...
      floatingActionButton: SpeedDial(
        ...
        children: [
          SpeedDialChild(
            ...
            label: 'dentify products from camera',
          ...
          SpeedDialChild(
            ...
            foregroundColor: Colors.white,
            label: 'IIdentify products from gallery',
            labelStyle: const TextStyle(fontSize: 18.0),
            onTap: () {
              setState(() {
  ...
  Widget task(Options option) {
    ...
    if (option== Options.checkout){
      List<PriceItem> updateShoppingListFromYolo(List<dynamic> yoloResults, List<PriceItem> shoppingList) {
  final detectedItems = <String, int>{}; 
  for (final item in yoloResults) {
    final name = item['tagName'].toLowerCase(); // Convert to lowercase for case-insensitive matching
    detectedItems[name] = (detectedItems[name] ?? 0) + 1; 
  }

  return shoppingList.map((item) {
    final detectedQuantity = detectedItems[item.name.toLowerCase()]; // Check in lowercase
    if (detectedQuantity != null && item.canEditQuantity) {
      return PriceItem(
        name: item.name,
        quantity: detectedQuantity, 
        itemCostCents: item.itemCostCents,
        canEditQuantity: true,
      );
    }
    return item; // No change if not detected or not editable
  }).toList();
}
      return MyDemoPage();
    }
...
class _YoloImageV5State extends State<YoloImageV5> {
  ...
  Future<void> loadYoloModel() async {
    await widget.vision.loadYoloModel(
        ...
        modelPath: 'lib/assets/best.tflite',
        modelVersion: "yolov5",
        quantization: false,
        numThreads: 2,
        useGpu: true);
